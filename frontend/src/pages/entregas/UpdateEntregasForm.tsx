...
const UpdateEntregasForm = ({ onSuccess, onError, onWarn, onEventoCreated, onReceptorCreated, onItemCreated, selectedEntrega, eventos, receptores, itens }: UpdateFormProps) => {
    ...
    const [eventoID, setEventoID] = useState(selectedEntrega.Evento.ID_Evento);
    const [receptorID, setReceptorID] = useState(selectedEntrega.Receptor.CPF);
    const { execute: criarEntrega, isLoading, error } = useMutation('/entrega', 'PUT');
    ...
    const [itensEntrega, setItensEntrega] = useState<ItemEntrega[]>(selectedEntrega.itensEntregues);
...
export default UpdateEntregasForm;